# -*- coding: utf-8 -*-
"""ui/dialogs/cat_dlg.py — Dialog configurare CAT cu PTT, S-meter, Split, Poll interval"""
import tkinter as tk
from tkinter import ttk
from ..theme import TH
from ...hardware.cat_engine import (CAT_PROTOCOLS, CAT_BAUD_DEFAULTS,
                                       CAT_SERIAL_DEFAULTS, CATEngine,
                                       PTT_METHODS, POLL_INTERVAL_DEFAULT)
from ... import i18n
...
class CATDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        gold = TH.get("gold", "#f39c12")
        accent = TH.get("accent", "#1a5276")
        ok_c   = TH.get("ok", "#1e8449")
        err_c  = TH.get("err", "#c0392b")

        nb = ttk.Notebook(self)
        nb.pack(fill="both", expand=True, padx=10, pady=8)

        tab_serial = tk.Frame(nb, bg=TH["bg"])
        tab_ptt    = tk.Frame(nb, bg=TH["bg"])
        tab_hamlib = tk.Frame(nb, bg=TH["bg"])
        tab_test   = tk.Frame(nb, bg=TH["bg"])
        nb.add(tab_serial, text="  Serial / COM  ")
        nb.add(tab_ptt,    text="  PTT & Poll  ")
        ...
        tk.Frame(p, bg=accent, height=1).grid(
            row=3, column=0, columnspan=4, sticky="ew", pady=6)
        tk.Label(p, text="Parametri seriali:", bg=TH["bg"], fg=gold,
        ...
        tk.Label(p, text=civ_hint, bg=TH["bg"], fg=gold,
                 font=("Consolas",8), justify="left").grid(
            row=7, column=2, columnspan=2, sticky="w", padx=4)

        # ═══ TAB 2 - PTT & Poll ════════════════════════════════════════
        q = tk.Frame(tab_ptt, bg=TH["bg"])
        q.pack(fill="both", expand=True, padx=12, pady=10)

        # ── PTT ────────────────────────────────────────────────────────
        tk.Label(q, text="Configurare PTT (Push-To-Talk)", bg=TH["bg"],
                 fg=gold, font=("Consolas",10,"bold")).grid(
            row=0, column=0, columnspan=4, sticky="w", pady=(0,8))

        tk.Label(q, text="Metodă PTT:", **lo).grid(row=1, column=0, sticky="w", pady=4)
        self._ptt_v = tk.StringVar(value=self._cfg.get("cat_ptt_method", "CAT"))
        ptt_cb = ttk.Combobox(q, textvariable=self._ptt_v, values=PTT_METHODS,
                               state="readonly", width=10, font=fn)
        ptt_cb.grid(row=1, column=1, sticky="w", padx=8, pady=4)
        ptt_cb.bind("<<ComboboxSelected>>", self._on_ptt_method)

        ptt_help = tk.Label(q, text="", bg=TH["bg"], fg=gold, font=fns, justify="left")
        ptt_help.grid(row=1, column=2, columnspan=2, sticky="w", padx=4)
        self._ptt_help_lbl = ptt_help
        self._update_ptt_help()

        tk.Label(q, text="Port PTT (opțional):", **lo).grid(row=2, column=0, sticky="w", pady=4)
        ports_all = CATEngine.list_ports() or [""]
        self._ptt_port_v = tk.StringVar(value=self._cfg.get("cat_ptt_port", ""))
        self._ptt_port_cb = ttk.Combobox(q, textvariable=self._ptt_port_v,
                                          values=["(același ca CAT)"] + ports_all,
                                          width=16, font=fn)
        self._ptt_port_cb.grid(row=2, column=1, sticky="w", padx=8, pady=4)
        tk.Label(q, text="Lasă gol = același port ca CAT.\nFolosește alt port dacă interfața PTT\ne separată de interfața CAT.",
                 bg=TH["bg"], fg=gold, font=("Consolas",8), justify="left").grid(
            row=2, column=2, columnspan=2, sticky="w", padx=4)

        tk.Frame(q, bg=accent, height=1).grid(
            row=3, column=0, columnspan=4, sticky="ew", pady=10)

        # ── Poll interval ───────────────────────────────────────────────
        tk.Label(q, text="Interval poll CAT", bg=TH["bg"],
                 fg=gold, font=("Consolas",10,"bold")).grid(
            row=4, column=0, columnspan=4, sticky="w", pady=(0,6))

        tk.Label(q, text="Poll la fiecare:", **lo).grid(row=5, column=0, sticky="w", pady=4)
        self._poll_v = tk.StringVar(
            value=str(self._cfg.get("cat_poll_sec", POLL_INTERVAL_DEFAULT)))
        poll_frame = tk.Frame(q, bg=TH["bg"])
        poll_frame.grid(row=5, column=1, columnspan=3, sticky="w", pady=4)
        poll_sb = ttk.Spinbox(poll_frame, from_=0.5, to=10.0, increment=0.5,
                               textvariable=self._poll_v, width=6, font=fn)
        poll_sb.pack(side="left")
        tk.Label(poll_frame, text=" secunde   (recomandat: 1-3s)", **lo).pack(side="left")

        tk.Label(q, text="Valori mici = actualizare rapidă dar mai mult trafic serial.\n"
                          "IC-735 la 1200 baud: folosiți ≥ 2.0s",
                 bg=TH["bg"], fg=gold, font=("Consolas",8), justify="left").grid(
        ...
        tk.Label(q,
                 text="NOTE PTT:\n"
                      "• CAT  = comandă nativă protocol (recomandată)\n"
                      "• RTS  = linia RTS a portului serial (interfețe simple)\n"
                      "• DTR  = linia DTR a portului serial\n"
                      "• None = PTT dezactivat (recepție only)\n\n"
                      "Dacă folosești RTS/DTR, asigură-te că interfața ta\n"
                      "suportă controlul liniei RTS/DTR (ex. SignaLink, microHAM).",
                 bg=TH["bg"], fg=TH["fg"], font=fns, justify="left").grid(
            row=8, column=0, columnspan=4, sticky="w")

        # ═══ TAB 3 - Hamlib cu selector model ═════════════════════════
        ...
        tk.Label(model_frame, text="Model (dublu-click = selecteaza):", bg=TH["bg"],
                 fg=TH["fg"], font=fns).pack(anchor="w")
        model_sb = tk.Scrollbar(model_frame, orient="vertical")
        self._model_lb = tk.Listbox(model_frame, width=36, height=9,
                                     bg=TH["entry_bg"], fg=TH["fg"], font=fns,
                                     selectbackground=ok_c,
                                     selectforeground="white", bd=0, activestyle="none",
        ...
        self._model_note = tk.Label(h, text="", bg=TH["bg"], fg=gold, font=fns, anchor="w")
        ...
        self._rigctld_lbl = tk.Label(h, text="", bg=TH["bg"], fg=gold,
        ...
        tk.Label(t, text="Test CAT live — citire, scriere, PTT, Split, S-meter",
                 bg=TH["bg"], fg=gold, font=("Consolas",10,"bold")).grid(
            row=0, column=0, columnspan=4, sticky="w", pady=(0,8))

        # ── Citire ─────────────────────────────────────────────────────
        tk.Label(t, text="Frecventa citita:", **lo).grid(row=1, column=0, sticky="w", pady=3)
        ...
        tk.Label(t, text="Mod citit:", **lo).grid(row=2, column=0, sticky="w", pady=3)
        ...
        tk.Button(t, text="Citeste acum", command=self._test_read,
                  bg=accent, fg="white", font=fn, padx=10).grid(
            row=1, column=3, rowspan=2, padx=12, sticky="ns")

        tk.Frame(t, bg=accent, height=1).grid(row=3, column=0, columnspan=4, sticky="ew", pady=8)

        # ── Scriere frecvență ──────────────────────────────────────────
        tk.Label(t, text="Trimite frecventa:", **lo).grid(row=4, column=0, sticky="w", pady=3)
        ...
        tk.Button(t, text="Trimite VFO-A", command=self._test_set_freq,
                  bg=ok_c, fg="white", font=fn, padx=6).grid(row=4, column=3, padx=6)

        # ── Split / VFO-B ──────────────────────────────────────────────
        tk.Label(t, text="Split VFO-B:", **lo).grid(row=5, column=0, sticky="w", pady=3)
        self._split_freq_e = tk.Entry(t, width=12, **eo)
        self._split_freq_e.grid(row=5, column=1, sticky="w", padx=8)
        tk.Label(t, text="kHz", **lo).grid(row=5, column=2, sticky="w")
        tk.Button(t, text="Trimite VFO-B", command=self._test_set_split,
                  bg=TH.get("btn_bg","#2c3e50"), fg="white", font=fn, padx=6).grid(
            row=5, column=3, padx=6)

        # ── Scriere mod ────────────────────────────────────────────────
        tk.Label(t, text="Trimite mod:", **lo).grid(row=6, column=0, sticky="w", pady=3)
        ...
        tk.Button(t, text="Trimite", command=self._test_set_mode,
                  bg=ok_c, fg="white", font=fn, padx=10).grid(row=6, column=3, padx=6)

        tk.Frame(t, bg=accent, height=1).grid(row=7, column=0, columnspan=4, sticky="ew", pady=8)

        # ── PTT test ───────────────────────────────────────────────────
        ptt_row = tk.Frame(t, bg=TH["bg"])
        ptt_row.grid(row=8, column=0, columnspan=4, sticky="w", pady=4)
        tk.Label(ptt_row, text="Test PTT:", **lo).pack(side="left", padx=(0,12))

        self._ptt_tx_btn = tk.Button(ptt_row, text="▶  TX  (PTT ON)",
                  command=self._test_ptt_on,
                  bg=err_c, fg="white", font=("Consolas",10,"bold"), padx=12)
        self._ptt_tx_btn.pack(side="left", padx=4)

        self._ptt_rx_btn = tk.Button(ptt_row, text="■  RX  (PTT OFF)",
                  command=self._test_ptt_off,
                  bg=ok_c, fg="white", font=("Consolas",10,"bold"), padx=12)
        self._ptt_rx_btn.pack(side="left", padx=4)

        self._ptt_state_lbl = tk.Label(ptt_row, text="", bg=TH["bg"],
                                        fg=gold, font=fn)
        self._ptt_state_lbl.pack(side="left", padx=8)

        tk.Frame(t, bg=accent, height=1).grid(row=9, column=0, columnspan=4, sticky="ew", pady=8)

        # ── S-meter ────────────────────────────────────────────────────
        smeter_row = tk.Frame(t, bg=TH["bg"])
        smeter_row.grid(row=10, column=0, columnspan=4, sticky="w", pady=4)
        tk.Label(smeter_row, text="S-meter:", **lo).pack(side="left", padx=(0,8))
        self._smeter_bar_v = tk.IntVar(value=0)
        self._smeter_bar = ttk.Progressbar(smeter_row, orient="horizontal",
                                            length=200, mode="determinate",
                                            variable=self._smeter_bar_v,
                                            maximum=100)
        self._smeter_bar.pack(side="left")
        self._smeter_lbl = tk.Label(smeter_row, text="  ---", bg=TH["bg"],
                                     fg=TH.get("ok","#27ae60"), font=("Consolas",11,"bold"),
                                     width=8)
        self._smeter_lbl.pack(side="left")
        tk.Button(smeter_row, text="Citeste", command=self._test_smeter,
                  bg=accent, fg="white", font=fns, padx=8).pack(side="left", padx=6)

        tk.Frame(t, bg=accent, height=1).grid(row=11, column=0, columnspan=4, sticky="ew", pady=8)

        # ── Log test ───────────────────────────────────────────────────
        tk.Label(t, text="Log test:", bg=TH["bg"], fg=gold, font=fns).grid(
            row=12, column=0, columnspan=4, sticky="w")
        self._test_log = tk.Text(t, height=5, bg=TH["entry_bg"], fg=TH["fg"],
                                  font=("Consolas",9), state="disabled",
                                  relief="sunken", bd=1)
        self._test_log.grid(row=13, column=0, columnspan=4, sticky="ew", pady=(2,0))
        tk.Button(t, text="Goleste log", command=self._clear_log,
                  bg=TH.get("btn_bg","#2c3e50"), fg="white", font=fns).grid(
            row=14, column=3, sticky="e", pady=4)

        # Switch la tab hamlib dacă e setat
        ...
        # Status + butoane
        self._status_lbl = tk.Label(self, text="", bg=TH["bg"],
                                     fg=TH.get("ok","#27ae60"),
                                     font=fn, wraplength=600, justify="center")
        self._status_lbl.pack(pady=(4,2))

        bf = tk.Frame(self, bg=TH["bg"])
        bf.pack(pady=6)
        tk.Button(bf, text="Conecteaza", command=self._connect,
                  bg=ok_c, fg="white", font=fn).pack(side="left", padx=4)
        tk.Button(bf, text="Deconecteaza", command=self._disconnect,
                  bg=err_c, fg="white", font=fn).pack(side="left", padx=4)
        tk.Button(bf, text=i18n.t("save"), command=self._save,
                  bg=accent, fg="white", font=fn).pack(side="left", padx=4)
        tk.Button(bf, text=i18n.t("cancel"), command=self.destroy,
                  bg=TH.get("btn_bg","#2c3e50"), fg="white", font=fn).pack(side="left", padx=4)

    # ─── PTT help text ────────────────────────────────────────────────────────

    def _update_ptt_help(self):
        m = self._ptt_v.get() if hasattr(self, "_ptt_v") else "CAT"
        hints = {
            "CAT":  "Comandă nativă protocol radio (recomandată)",
            "RTS":  "Linia RTS a portului serial → interfețe simple/VOX",
            "DTR":  "Linia DTR a portului serial → interfețe simple/VOX",
            "None": "PTT dezactivat — numai recepție",
        }
        if hasattr(self, "_ptt_help_lbl"):
            self._ptt_help_lbl.config(text=hints.get(m, ""))

    def _on_ptt_method(self, e=None):
        self._update_ptt_help()
    ...
    def _check_connected(self, op="") -> bool:
        if not self._cat.connected:
            self._log(f"CAT neconectat{' — ' + op if op else ''} → apasă Conectează")
            return False
        return True

    def _test_read(self):
        if not self._check_connected("citire"):
            return
    ...
    def _test_set_freq(self):
        if not self._check_connected("set freq"):
            return
        khz = self._send_freq_e.get().strip()
        if not khz:
            self._log("Introdu o frecventa în kHz")
            return
        ok = self._cat.set_freq(khz)
        self._log(f"{'OK' if ok else 'ERR'}  Frecventa VFO-A trimisa: {khz} kHz")

    def _test_set_split(self):
        if not self._check_connected("split VFO-B"):
            return
        khz = self._split_freq_e.get().strip()
        if not khz:
            self._log("Introdu frecventa VFO-B în kHz")
            return
        ok = self._cat.set_split_freq(khz)
        self._log(f"{'OK' if ok else 'ERR'}  Frecventa VFO-B (split) trimisa: {khz} kHz")

    def _test_set_mode(self):
        if not self._check_connected("set mode"):
            return
        mode = self._send_mode_v.get()
        ok = self._cat.set_mode(mode)
        self._log(f"{'OK' if ok else 'ERR'}  Mod trimis: {mode}")

    def _test_ptt_on(self):
        ok = self._cat.ptt_on()
        state = "TX ACTIV" if ok else "ERR PTT"
        fg = TH.get("err","#e74c3c") if ok else TH.get("warn","#f39c12")
        self._ptt_state_lbl.config(text=f"  ● {state}", fg=fg)
        self._log(f"{'OK' if ok else 'ERR'}  PTT ON → {self._cat.ptt_method}")

    def _test_ptt_off(self):
        ok = self._cat.ptt_off()
        fg = TH.get("ok","#27ae60")
        self._ptt_state_lbl.config(text="  ○ RX", fg=fg)
        self._log(f"{'OK' if ok else 'ERR'}  PTT OFF → {self._cat.ptt_method}")

    def _test_smeter(self):
        if not self._check_connected("S-meter"):
            return
        val = self._cat.get_smeter()
        if val is None:
            self._smeter_lbl.config(text="  N/A")
            self._smeter_bar_v.set(0)
            self._log("S-meter: nu este suportat de acest protocol/radio")
            return
        self._smeter_bar_v.set(val)
        # Conversie % → S-unit aproximativ
        if val < 10:    s = "S0"
        elif val < 20:  s = "S1-2"
        elif val < 30:  s = "S3-4"
        elif val < 40:  s = "S5-6"
        ...
        self._smeter_lbl.config(text=f"  {val}% {s}")
        self._log(f"OK  S-meter: {val}% ≈ {s}")
    ...
    def _disconnect(self):
        ...
        self._ptt_state_lbl.config(text="")
        self._smeter_bar_v.set(0)
        self._smeter_lbl.config(text="  ---")
    ...
    def _collect(self) -> dict:
        ...
        try:
            ...
            hamlib_model = 3073
        try:
            poll_sec = float(self._poll_v.get())
            if poll_sec < 0.5:
                poll_sec = 0.5
        except (ValueError, AttributeError):
            poll_sec = POLL_INTERVAL_DEFAULT
        ptt_port = self._ptt_port_v.get() if hasattr(self, "_ptt_port_v") else ""
        if ptt_port == "(același ca CAT)":
            ptt_port = ""
        return {
            **self._cfg,
            ...
            "cat_ptt_method":   self._ptt_v.get(),
            "cat_ptt_port":     ptt_port,
            "cat_poll_sec":     poll_sec,
